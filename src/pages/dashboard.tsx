...
import { getServiceTypes } from 'apiSdk/service-types';
import { ServiceTypeInterface } from 'interfaces/service-type';
import { getServiceHistories } from 'apiSdk/service-histories';
import { ServiceHistoryInterface } from 'interfaces/service-history';
import { getLaptopInventories } from 'apiSdk/laptop-inventories';
import { LaptopInventoryInterface } from 'interfaces/laptop-inventory';

import { LaptopInventoryListPage as ShowCaseList } from 'pages/laptop-inventories';

function DashboardPage() {
  const { session } = useSession();

  const serviceTypeStat = useSWR<PaginatedInterface<ServiceTypeInterface>>(
    () => `/service-types`,
    () => getServiceTypes({ limit: 1, offset: 0 }),
  );

  const serviceHistoryStat = useSWR<PaginatedInterface<ServiceHistoryInterface>>(
    () => `/service-histories`,
    () => getServiceHistories({ limit: 1, offset: 0 }),
  );

  const laptopInventoryStat = useSWR<PaginatedInterface<LaptopInventoryInterface>>(
    () => `/laptop-inventories`,
    () => getLaptopInventories({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Service Types',
      result: serviceTypeStat?.data?.totalCount,
      isLoading: serviceTypeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Service Histories',
      result: serviceHistoryStat?.data?.totalCount,
      isLoading: serviceHistoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Laptop Inventories',
      result: laptopInventoryStat?.data?.totalCount,
      isLoading: laptopInventoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Laptop Inventories
                </Text>
                <NextLink href={`laptop-inventories`} passHref legacyBehavior>
                  <Button
