...
import { getServiceHistoryById, updateServiceHistoryById } from 'apiSdk/service-histories';
...
function ServiceHistoryEditPage() {
  ...
  const handleSubmit = async (values: ServiceHistoryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateServiceHistoryById(id, values);
      mutate(updated);
      resetForm();
