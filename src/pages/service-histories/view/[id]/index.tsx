...
import { getServiceHistoryById } from 'apiSdk/service-histories';
import { ServiceHistoryInterface } from 'interfaces/service-history';

function ServiceHistoryViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ServiceHistoryInterface>(
    () => (id ? `/service-histories/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('service_history', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/service-histories/edit/${id}`} passHref legacyBehavior>
                    <Button
